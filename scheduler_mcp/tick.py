...
async def _run_job(
    ledger: Ledger, dispatcher: Dispatcher, sem: asyncio.Semaphore, owner: str, job: dict
) -> None:
    """Execute un job verrouille : idempotence, dispatch borne, cloture, avance.
...
async def tick_once(
    ...
    for job in due:
        if not await ledger.acquire_lock(job["id"], owner, cfg.lock_ttl_seconds, now):
            ...
            continue
        tasks.append(asyncio.create_task(_run_job(ledger, dispatcher, sem, owner, job)))
    if tasks:
        log.info("tick.dispatched", count=len(tasks))
    return tasks


async def tick_loop(cfg: Config, ledger: Ledger, dispatcher: Dispatcher) -> None:
    ...
    while True:
        try:
            tasks = await tick_once(cfg, ledger, dispatcher, sem, owner)
            for task in tasks:
                pending.add(task)
